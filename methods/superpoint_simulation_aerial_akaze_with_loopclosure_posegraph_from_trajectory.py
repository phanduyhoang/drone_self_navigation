...
def akaze_extract(gray: np.ndarray, max_kp: int):
    akaze = cv2.AKAZE_create()
    kpts_cv, desc = akaze.detectAndCompute(gray, None)
    if not kpts_cv or desc is None or len(kpts_cv) == 0:
        return None, None, None
    # downselect to max_kp by response
    if len(kpts_cv) > max_kp:
        idx_sorted = np.argsort([-kp.response for kp in kpts_cv])[:max_kp]
        kpts_cv = [kpts_cv[i] for i in idx_sorted]
...
def akaze_match(desc0: np.ndarray, desc1: np.ndarray, ratio=0.8):
...
def calibrate_convention_akaze(big, traj_xy_yaw, window, max_kp, yaw_sign,
    ...
    def median_err(cam_sign, yaw_use):
        ...
        for i in range(0, straight_end):
            ...
            kpts, desc, scores = akaze_extract(gray, max_kp)
            if kpts is None:
                prev_kpts = None
                prev_desc = None
                continue
            if prev_kpts is not None and prev_desc is not None:
                i0, i1 = akaze_match(prev_desc, desc)
...
def main():
    ...
    cam_sign, yaw_use_auto = calibrate_convention_akaze(
    ...
    print("=== Running VO (AKAZE) with loop closure ===")
    ...
    for loop_i in range(args.loops):
        for i, (gx, gy, yaw_cur) in enumerate(traj_xy_yaw):
            ...
            stab = rotate_square(win, angle_rad=(-cam_sign * yaw_used), borderValue=0)
            # Fix borderValue arg name for cv2; use border_value param consistency
            stab = rotate_square(win, angle_rad=(-cam_sign * yaw_used), border_value=0)
            gray = cv2.cvtColor(stab, cv2.COLOR_BGR2GRAY)
            kpts, desc, scores = akaze_extract(gray, args.max_kp)
            ...
            if (not args.no_loop_closure) and (frame_idx % args.lc_check_every == 0):
                if (frame_idx - last_lc_frame) >= args.lc_cooldown and len(kf_db) > 0 and kpts is not None and desc is not None:
                    ...
                    for kf in candidates:
                        i0, i1 = akaze_match(kf["desc"], desc)
    ...
    traj_out = args.save_traj_map if args.save_traj_map else "traj_map_akaze.png"
    overlay_out = args.save_gt_overlay if args.save_gt_overlay else "gt_overlay_akaze.png"
