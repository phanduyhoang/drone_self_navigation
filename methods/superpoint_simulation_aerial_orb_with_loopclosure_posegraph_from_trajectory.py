...
# -----------------------------
def orb_extract(gray: np.ndarray, max_kp: int):
    orb = cv2.ORB_create(nfeatures=max_kp)
    kpts_cv, desc = orb.detectAndCompute(gray, None)
...
def orb_match(desc0: np.ndarray, desc1: np.ndarray):
    bf = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=True)
    matches = bf.match(desc0, desc1)
    matches = sorted(matches, key=lambda m: m.distance)
    if len(matches) == 0:
        return None, None
    p0 = np.array([m.queryIdx for m in matches], dtype=np.int32)
    p1 = np.array([m.trainIdx for m in matches], dtype=np.int32)
    return p0, p1

# -----------------------------
# Estimate translation from matched pairs
# -----------------------------
...
# -----------------------------
# Calibrate cam_sign + yaw_use using ORB
# -----------------------------
def calibrate_convention_orb(big, traj_xy_yaw, window, max_kp, yaw_sign,
    ...
    def median_err(cam_sign, yaw_use):
        ...
        for i in range(0, straight_end):
            ...
            kpts, desc, scores = orb_extract(gray, max_kp)
            if kpts is None:
                prev_kpts = None
                prev_desc = None
                continue
            if prev_kpts is not None and prev_desc is not None:
                i0, i1 = orb_match(prev_desc, desc)
...
def main():
    ...
    # keep SuperGlue params for CLI parity (ignored)
    ...
    cam_sign, yaw_use_auto = calibrate_convention_orb(
    ...
    print("=== Running VO (ORB) with loop closure ===")
    ...
    for loop_i in range(args.loops):
        for i, (gx, gy, yaw_cur) in enumerate(traj_xy_yaw):
            ...
            gray = cv2.cvtColor(stab, cv2.COLOR_BGR2GRAY)
            kpts, desc, scores = orb_extract(gray, args.max_kp)
            ...
            # keyframe DB (store CPU descriptors)
            ...
            if args.log_every > 0 and (frame_idx % args.log_every == 0):
                ...
                print(f"[{loop_i+1}/{args.loops}] frame={frame_idx}/{total_frames} "
                      ...
                      f"est=({est_xy[0]:.1f},{est_xy[1]:.1f}) yaw_used={math.degrees(yaw_used_val):.1f}°")
    ...
    # auto-named outputs if not provided
    traj_out = args.save_traj_map if args.save_traj_map else "traj_map_orb.png"
    overlay_out = args.save_gt_overlay if args.save_gt_overlay else "gt_overlay_orb.png"
